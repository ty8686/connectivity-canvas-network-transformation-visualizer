  ...
  return (
    <aside className="absolute left-6 top-6 z-20 w-16 md:w-44 flex flex-col gap-4 font-sans">
      <div className="sketchy-card p-3 bg-white/95 backdrop-blur-sm shadow-xl border-[#2D2D2D]">
        <h3 className="font-illustrative text-md mb-3 hidden md:block border-b border-border pb-2 text-[#2D2D2D]">Infrastructure</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {TOOLS.map((tool) => (
            ...
            >
              ...
              <span className="text-[9px] mt-1 font-bold uppercase tracking-tighter text-[#2D2D2D] hidden md:block">{tool.label}</span>
            </div>
          ))}
        </div>
      </div>
      <div className="sketchy-card p-3 bg-white/95 backdrop-blur-sm hidden md:block border-[#2D2D2D]">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <p className="text-[10px] text-[#2D2D2D] uppercase font-black tracking-tighter">Usage</p>
        </div>
        <p className="text-[11px] leading-tight text-[#2D2D2D] opacity-80 font-medium">Drag icons to the canvas to map your infrastructure.</p>
      </div>
    </aside>
  );
}