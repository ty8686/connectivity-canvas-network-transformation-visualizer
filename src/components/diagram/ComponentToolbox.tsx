...
export function ComponentToolbox() {
  ...
  return (
    <aside className="absolute left-6 top-6 z-20 w-16 md:w-44 flex flex-col gap-4">
      <div className="sketchy-card p-3 bg-white/95 backdrop-blur-sm shadow-xl">
        <h3 className="font-illustrative text-md mb-3 hidden md:block border-b border-border pb-2">Infrastructure</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {TOOLS.map((tool) => (
            ...
            >
              ...
              <span className="text-[9px] mt-1 font-bold uppercase tracking-tighter text-muted-foreground hidden md:block">{tool.label}</span>
            </div>
          ))}
        </div>
      </div>
      <div className="sketchy-card p-3 bg-white/95 backdrop-blur-sm hidden md:block">
        <div className="flex items-center gap-1.5 mb-1">
          <div className="w-1.5 h-1.5 rounded-full bg-[#F48120] animate-pulse" />
          <p className="text-[10px] text-muted-foreground uppercase font-black tracking-tighter">Usage</p>
        </div>
        <p className="text-[11px] leading-tight text-foreground/80">Drag icons to the canvas to map your infrastructure.</p>
      </div>
    </aside>
  );
}